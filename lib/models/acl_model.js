...
var ACLModel = BaseModel.extend({
    ...
    _.each(this.schema.properties, function (prop, key) {
      if (!prop.permissions && objectAccess) {
        ...
        if (mask.assert(roles, action)) {
          debug.log('adding key %s because of %s',key,roles.join(','));
          keys.push(key);
        }
      }
    }, this);
  ...
  validate: function (attrs, options) {
    ...
    var actor = options.actor || options.user || options.model;
    debug.log('validate %s: %s %s %s', this.url(), action, JSON.stringify(changedAttrs), JSON.stringify(options));
