...
var ACLModel = BaseModel.extend({
 ...
 //   debug.log('Adding roles %s to model %s', _.uniq(this.roles.concat(roles)), this.url());
  ...
  toJSON: function(options) {
    ...
    // if option `includePermissions` is true, include main level permissions in the JSON
    if (options.includePermissions && this.acl) {
      json.permissions = this.acl.permissions;
    }
    return json;
  },
...
//    debug.log('canAccess %s %s', action, model.url());
  ...
  propertiesWithAccess: function(action, model) {
    ...
    debug.log('propertiesWithAccess giving out keys: %s to %s(%s) to %s(%s)',
      keys.join(','), model.type, model.id, self.type, self.id);
    return keys;
  },

  /**
   * Override default validation by adding ACL check. Checks if actor has access to proceed with given action.
   */
  validate: function(attrs, options) {
    ...
    if (this.acl) {
      ...
      if ((!canAccess && changedAttrs) || (noAccessTo && noAccessTo.length > 0 && action !== 'validate')) {
