...
var ACLModel = BaseModel.extend({
  ...
  fetch: function(options) {
    ...
    options.action = 'read';
    options.actor = options.actor || this.actor;
    debug.trace('ACLModel(%s).fetch %s actor present: %d',this.type||'', options.action, options.actor !== undefined);
  ...
  destroy: function(options) {
    ...
    options.actor = options.actor || this.actor;
    debug.trace('ACLModel(%s).destroy %s actor present: %d',this.type||'', options.action, options.actor !== undefined);
  ...
  getRoles: function(actor) {
    ...
    _.each(this.relationDefinitions, function (value, key) {
      ...
      if (actor.type === value.Class.prototype.type
        && actor.has(refs[0])
        && this.has(refs[1])
  ...
  validate: function (attrs, options) {
    ...
    debug.trace('validate: %s',JSON.stringify(options), attrs, changedAttrs);
    ...
    if (this.acl) {
      ...
      if ((!canAccess && changedAttrs) || (noAccessTo && noAccessTo.length > 0) ) {
        ...
        var error = new errors.ForbiddenError('No access to ' + action + ' fields:' + noAccessTo);
    ...
    debug.log('ACL valid, proceeding with basemodel validation');
