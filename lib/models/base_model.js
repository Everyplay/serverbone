...
var BaseModel = SchemaModel.extend({
  ...
  save: function(key, val, options) {
    ...
    return this.preSave(opt).then(function() {
      return PromisedModel.prototype.save.call(self, key, val, options).then(function(model) {
        return self.afterSave(opt).then(function() {
          return model;
        }, function(err) {
          if (opt.error) opt.error.call(opt, self, err);
        });
      });
    }, function(err) {
      if (opt.error) opt.error.call(opt, self, err);
