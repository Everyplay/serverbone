...
var BaseModel = SchemaModel.extend({
  ...
  save: function(key, val, options) {
    debug.trace('save %s', JSON.stringify(val));
    var opts = options || {};
    var self = this;
  ...
  validate: function(attrs, options) {
    ...
    debug.log('BaseModel.validate(%s)',this.type, validationErrors);
  ...
  preFetch: function() {
    debug.log('preFetch %s', JSON.stringify(arguments));
    return when.resolve();
  },

  afterFetch: function() {
    debug.log('afterFetch %s', JSON.stringify(arguments));
    return when.resolve();
  },
