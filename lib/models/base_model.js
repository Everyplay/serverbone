...
var BaseModel = SchemaModel.extend({
  ...
  save: function(key, val, options) {
    debug.trace('save %s actor',JSON.stringify(val));
    var opts = options || {};
    var self = this;
  ...
  fetch: function(options) {
    debug.trace('fetch %s',JSON.stringify(options));
    var self = this;
    var promise = PromisedModel.prototype.fetch.apply(this, arguments);
    promise
      .then(function() {
  ...
  destroy: function(options) {
    debug.trace('destroy %s',JSON.stringify(options));
    options =  options || {};
    var fns = [
  ...
  validate: function(attrs, options) {
    debug.trace('validate %s %s',JSON.stringify(attrs), JSON.stringify(options));
    options = options || {};
  ...
  fetchRelations: function(options) {
    ...
    _.each(self.relationDefinitions, function(relationAttributes, relationKey) {
      if (_.isArray(options.onlyRelations)) {
        if (options.onlyRelations.indexOf(relationKey) === -1) {
          return;
        }
      }
      relation = self.get(relationKey);
    ...
    return parallel(fns)
      .then(function relationsFetched() {
        // fetch also nested relations
        if (options.nestedRelations) {
          fns = [];
          _.each(options.nestedRelations, function(rel) {
            var parentRelation = self.get(rel.relation);
            if (parentRelation) {
              fns.push(_.bind(parentRelation.fetchAll, parentRelation));
            }
          });
          return parallel(fns).then(function() {
            self.fetchStatus = 'all_fetched';
            return self;
          }, error);
        } else {
  ...
  preSave: function() {
    debug.log('preSave %s', JSON.stringify(arguments));
    return when.resolve();
  },

  afterSave: function() {
    debug.log('afterSave %s', JSON.stringify(arguments));
    return when.resolve();
  },
  ...
  attributesChanged: function(model, options) {
    this._changedSinceSync = _.extend(this._changedSinceSync, this.changedAttributes());
  },
