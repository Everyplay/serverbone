...
var BaseCollection = SchemaCollection.extend({
  ...
  create: function(data, opts) {
    if (data) {
      var defaultAttrs = {};
      var keyExistsFn = (data instanceof Backbone.Model) ?
                        data.has : data.hasOwnProperty;
      _.each(this.defaultOptions && this.defaultOptions.where, function(val, key) {
        if (_.isObject(val)
            && !(val.constructor && val.constructor.name === 'ObjectID')) {
          return;
        }
        if (keyExistsFn.call(data, key)) return;
        defaultAttrs[key] = val;
      });
      if (!_.isEmpty(defaultAttrs)) {
        if (data instanceof Backbone.Model) {
          data = data.clone();
          data.set(defaultAttrs);
        } else {
          data = _.clone(data);
          _.defaults(data, defaultAttrs);
        }
      }
    }
    ...
    return fetchFn()
      .then(function() {
        ...
        self.models.forEach(function(model) {
          fns.push(_.bind(applyToModel, self, model));
        });
